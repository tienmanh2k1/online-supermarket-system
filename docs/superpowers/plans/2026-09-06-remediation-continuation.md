...
- [x] Thêm endpoint tests với fake verifier/processor: unknown provider 401, malformed 400, invalid signature 401, null data 400, safe config failure; mọi reject không gọi processor (16 endpoint tests). Test đủ bốn outcome (Processed/AlreadyProcessed/PaymentNotFound/Conflict) và không lộ verifier detail — có test `Response_DoesNotLeakSignatureOrErrorDetail` + mới `Verifier_ErrorCode_IsNotEchoed_IntoResponse`.
- [x] Sửa thứ tự error mapping: MALFORMED_CALLBACK được xét trước nhánh chung `!IsValidSignature` (CheckoutEndpoints.cs:321–330). Validate Guid/event/amount của result trước delegation; branch `callback.ErrorCode is not null` giờ trả `MALFORMED_CALLBACK` generic, không echo ErrorCode tùy ý.
- [x] Trong transaction: duplicate check; chọn latest payment theo `CreatedAtUtc` + tie-breaker `Id`; validate provider/method/amount; load order/items/history; chỉ chấp nhận order Pending/Confirmed trước khi attach callback.
- [x] Success chỉ Pending→Confirmed (Confirmed giữ nguyên + history). Failure: Release đủ số lượng đúng operation key `order:{id}:inventory:{id}:release`, release promotion usage đúng một lần, order→Cancelled. Missing inventory throw → rollback toàn bộ (không dùng `.Where(inventories.ContainsKey)` bỏ qua).
- [x] Thêm tests: order Completed/Cancelled/Failed không bị chuyển ngược (theory TerminalOrder), wrong amount/provider không ghi callback row (assert PaymentCallbacks.Count==0), nhiều payment chọn latest, missing inventory không partial effect, failed callback trả lại promotion usage, exception giữa transaction rollback payment/order/ledger/promotion.
- [x] Không trả outcome rồi để entity Added/Modified treo: early-return nằm trong `await using var transaction` nên dispose sẽ rollback; guard mới ở đầu `ProcessAsync` từ chối kết quả unverified (`InvalidOperationException`), test `UnverifiedResult_IsRejected_WithNoEffects` chứng minh không hiệu ứng.
